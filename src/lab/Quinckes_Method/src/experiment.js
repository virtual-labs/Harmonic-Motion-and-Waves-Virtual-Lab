...
var quinckes_stage, quinckes_container, magneticfield_container, exp_canvas;
...
function directiveFunction() {
	return {
		...
		link: function(scope, element, attrs) {
			...
			loadingProgress(queue, quinckes_stage, exp_canvas.width)
			...
			{ /** Background - Quincke's containerr */ 
				id: "background_022",
				src: "././images/bg_022.svg",
				type: createjs.LoadQueue.IMAGE
			},
			{ /** Background - Quincke's containerr */ 
				id: "background_023",
				src: "././images/bg_023.svg",
				type: createjs.LoadQueue.IMAGE
			},
			{ /** Background - Quincke's containerr */ 
				id: "background_024",
				src: "././images/bg_024.svg",
				type: createjs.LoadQueue.IMAGE
			},
			{ /** Background - Quincke's containerr */ 
				id: "background_025",
				src: "././images/bg_025.svg",
				type: createjs.LoadQueue.IMAGE
			},
			...
			function handleComplete() {
				...
				/** Background- Magnetic field current container*/
				loadImages(queue.getResult("background_01"), "background1", 0, 0, "", 0, 1, magneticfield_container); 
				loadImages(queue.getResult("background_011"), "background11", 0, 0, "", 0, 1, magneticfield_container); 
				/** Background - Quincke's containerr */
				loadImages(queue.getResult("background_02"), "background2", 0, 0, "", 0, 1,  quinckes_container); 
				loadImages(queue.getResult("background_021"), "background21", 0, 0, "", 0, 1,  quinckes_container); 
				loadImages(queue.getResult("background_022"), "background22", 0, 0, "", 0, 1,  quinckes_container); 
				loadImages(queue.getResult("background_023"), "background23", 0, 0, "", 0, 1,  quinckes_container); 
				loadImages(queue.getResult("background_024"), "background24", 0, 0, "", 0, 1,  quinckes_container); 
				loadImages(queue.getResult("background_025"), "background25", 0, 0, "", 0, 1,  quinckes_container); 
				...
				/** Probe not connected image - Magnetic field current container*/
				loadImages(queue.getResult("probe_not_connect"), "probe_not_connect", 421, 502, "", 0, 1, magneticfield_container); 
				/** Probe connected image - Magnetic field current container*/
				loadImages(queue.getResult("probe_connect"), "probe_connect", 338, 232, "", 0, 1, magneticfield_container);
				/** Tuner button image- Magnetic field current container */
				loadImages(queue.getResult("turner1"), "turner1", 190, 530, "", 0, 1, current_container); 
				/** Tuner button image- Magnetic field current container */
				loadImages(queue.getResult("turner2"), "turner2", 190, 530, "", 0, 1,  current_container); 
				/** Tuner button rotate image - Magnetic field current container */
				loadImages(queue.getResult("turnerrotate"), "turnerrotate", 237, 583, "", 0, 1, current_container); 
				/** Tube zoom - Quincke's container */
				...
				setText("currentVal", 101, 570, "0.5", "black", 2, current_container); /** Current value label */
				setText("currentSymbol", 160, 573, "A", "black", 1, current_container); /** Current symbol label */
				setText("gaussianVal", 496, 578, "0.000", "black", 2, magneticfield_container); /** Gaussian label */
				/** Gaussian initial value label */
				setText("gaussianInitialVal", 496, 578, "0.000", "black", 2, magneticfield_container); 
				/** Constant current supply label */
				setText("currentSupplyLabel", 70, 592, _("CONSTANT CURRENT SUPPLY"), "black", 0.6, current_container);
				/** Gauss meter label */
				setText("gaussMeterLabel", 485, 600, _("GAUSS METER"), "black", 0.6, magneticfield_container); 
				...
				/** Rectangle shape for masking tube solution */
...
function setText(name, textX, textY, value, color, fontSize, container) {
	...
	if( name=="currentVal" || name=="gaussianVal" || name=="gaussianInitialVal" ){
		_text.font = "1.9em digiface";
	}
	_text.name = name;
	_text.text = value;
	_text.color = color;
...
function changeSetupFn(scope) {
	...
	if (_setup_index == 0) { /** Magnetic field Vs Current */ 
		...
		drawRectangle(reverse_arrow, "reverse_arrow", 280, 520, 30, 30, "black", current_container, scope, "pointer"); /** Rectacle shape for clicking on the forward arrow on constant current supply machine*/
		drawRectangle(forward_arrow, "forward_arrow", 330, 520, 30, 30, "black", current_container, scope, "pointer"); /** Rectacle shape for clicking on the reverse arrow on constant current supply machine*/
  		current_container.getChildByName("currentVal").x=160;/** Change current value label position */
  		current_container.getChildByName("currentSymbol").x=215;/** Change current symbol position */
  		current_container.getChildByName("currentSupplyLabel").x=125;/** Change current symbol label position */
		current_container.getChildByName("turner1").x=240;/** Change tuner 1 position */
		current_container.getChildByName("turner2").x=240;/** Change tuner 2 position */
		current_container.getChildByName("turnerrotate").x=290; /** Change tuner rotate image position */		
   }
   zoom_container.alpha = 0;
   quinckes_stage.update(); /** Stage update function in a timer */
}
